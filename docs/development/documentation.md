...
- 开发过程中在 `PropertyTable.xaml.cs` 登记的新增或重命名属性，其引入版本统一标记为“未发布”。日常功能提交不得提前填写预计版本号；只有准备正式发版时，才将本次发布涉及的“未发布”项统一替换为实际版本号。
...
- [ ] Gallery 新增或重命名属性在 `PropertyTable.xaml.cs` 中标记为“未发布”，没有提前填写预计版本号。
