...
3. 用最小实现使测试通过。
4. 在不降低断言强度的前提下重构。
5. 运行受影响测试。组件库的日常新增、修改或删除只验证受影响的测试项目、测试类或测试方法，并选择一个当前主目标框架执行；不要因此在本地运行全框架测试矩阵。

只有改动目标框架配置、跨框架兼容层、构建或打包基础设施，进入发布验证，或者用户明确要求时，才运行全框架测试。远程 CI 在隔离 Runner 中执行的框架矩阵不受此本地验证规则影响。
