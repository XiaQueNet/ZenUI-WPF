...
ZenUI 以 Zen Style 为默认设计气质：通过克制的视觉与交互表达减少干扰，让使用者自然、平静地完成当前任务。这里的“极简”不是简单删除元素，而是删除噪声、保留必要信息，并以清晰、从容的方式呈现。

- 界面保持清爽，优先使用留白、排版和对齐建立层级，避免依赖装饰堆叠。
- 配色柔和而有节制；强调色和状态色只用于传达焦点、操作与明确语义。
- 动画用于解释状态和空间关系，持续时间与幅度保持克制，不以吸引注意为目的。
- 控件默认视觉只突出当前任务所需的信息和操作，次要能力按需呈现。
- 不用阴影、边框、色块或圆角反复包裹同一层级，避免视觉喧宾夺主。
- 极简不能以牺牲可读性、可访问性、状态辨识或操作效率为代价。

作为通用组件库，ZenUI 的目标是优化 WPF 控件的默认体验，而不是缩减 WPF 已有能力：
...
目前提供 Button、TextBox、NumberBox、PasswordBox、Switch、CheckBox、RadioButton、RadioGroup、ComboBox、ListBox、Calendar、DatePicker、TimePicker、DataGrid、Slider、ProgressBar、Alert、Expander、Popover、ContextMenu 等常用控件。所有控件均自带默认主题，并覆盖悬停、焦点、选中和禁用等常见交互状态。
